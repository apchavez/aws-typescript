...
awslocal sqs create-queue --queue-name appointments-country-dlq
awslocal sqs create-queue --queue-name appointments-confirmaciones-dlq

COUNTRY_DLQ_ARN=$(awslocal sqs get-queue-attributes \
  --queue-url http://localhost:4566/000000000000/appointments-country-dlq \
...
awslocal sqs create-queue --queue-name appointments-country \
  --attributes "{\"RedrivePolicy\":\"{\\\"deadLetterTargetArn\\\":\\\"$COUNTRY_DLQ_ARN\\\",\\\"maxReceiveCount\\\":\\\"3\\\"}\"}"
...
COUNTRY_QUEUE_ARN=$(awslocal sqs get-queue-attributes \
  --queue-url http://localhost:4566/000000000000/appointments-country \
  --attribute-names QueueArn --query Attributes.QueueArn --output text)

awslocal sns subscribe \
  --topic-arn "$TOPIC_ARN" \
  --protocol sqs \
  --notification-endpoint "$COUNTRY_QUEUE_ARN" \
