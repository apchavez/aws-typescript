...
describe("DynamoAppointmentStateRepo (unit)", () => {
  ...
  test("markCompleted -> UpdateCommand sets status to completed", async () => {
    ddbMock.on(UpdateCommand).resolves({});
    await repo.markCompleted("u1");
    ...
    expect((input.ExpressionAttributeValues as Record<string, unknown>)[":c"]).toBe("completed");
