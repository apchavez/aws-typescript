...
describe("appointment_country handler", () => {
  ...
  test("handler -> calls svc.process for a PE payload", async () => {
    mockProcess.mockResolvedValue(undefined);

    await handler(sqsEvent(appointmentPE), ctx, cb);

    expect(mockProcess).toHaveBeenCalledTimes(1);
    expect(mockProcess).toHaveBeenCalledWith(appointmentPE);
  });

  test("handler -> calls svc.process for a CL payload", async () => {
    mockProcess.mockResolvedValue(undefined);

    await handler(sqsEvent(appointmentCL), ctx, cb);

    expect(mockProcess).toHaveBeenCalledTimes(1);
    expect(mockProcess).toHaveBeenCalledWith(appointmentCL);
  });
