...
function setBaseEnv() {
  process.env.RDS_HOST = "rds-host";
  process.env.RDS_USER = "admin";
  process.env.RDS_DATABASE = "appointments";
...
describe("MySQLCountryBookingRepo", () => {
  ...
  test("book -> fetches the password from SSM and caches it across bookings", async () => {
  ...
  test("book -> reuses the same pool for repeated bookings", async () => {
    process.env.RDS_PASSWORD = "env-password";
  ...
  test("book -> honors RDS_PORT override instead of the 3306 default", async () => {
    process.env.RDS_PASSWORD = "env-password";
    process.env.RDS_PORT = "3307";
    const { repoModule, mysql } = await loadRepoModule();
    const repo = new repoModule.MySQLCountryBookingRepo();
    ...
    expect(mysql.createPool as jest.Mock).toHaveBeenCalledWith(
      expect.objectContaining({ host: "rds-host", port: 3307, database: "appointments" })
    );
  });
