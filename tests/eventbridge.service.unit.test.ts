...
describe("EventBridgeConfirmationBus", () => {
  ...
  test("send -> publishes an AppointmentConfirmed event with the given detail", async () => {
    ...
    expect(entry?.Source).toBe("appointment-service");
  ...
  test("send -> propagates errors from the EventBridge client", async () => {
    ...
    await expect(bus.send("u2")).rejects.toThrow("throttled");
  });
});
