...
> **Dev sin instalar el toolchain:** `./docker/dev.sh test` corre build/test dentro del mismo contenedor que usa CI, y `./docker/dev.sh terraform` valida el IaC, sin necesitar nada más que Docker instalado localmente (`./docker/dev.sh` sin argumentos muestra las opciones).
