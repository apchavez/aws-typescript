...
Los mensajes que fallan después de 3 reintentos se enrutan a una Dead Letter Queue (retención de 14 días) por cada cola SQS. `countryISO` se conserva como dato del dominio (viaja en el mensaje y se persiste en MySQL) pero ya no determina el enrutamiento — hay una única cola/worker para ambos países.
...
│   ├── appointment_country.ts      Country worker único (PE + CL comparten cola y lógica)
...
│   │   └── IMessageBus.ts
│   └── types.ts                    CountryISO | Status | Role
├── infra/
...
| `SQS_COUNTRY_URL` / `SQS_COUNTRY_ARN` | Cola SQS del country worker (PE + CL) |
...
Levanta equivalentes locales de DynamoDB, SQS, SNS, SSM y EventBridge — más un contenedor MySQL para el country worker — sin necesitar una cuenta de AWS.
...
| Suscripción SNS → SQS | `appointments-country` (sin filtro, recibe PE y CL) |
...
- Un único worker Lambda para ambos países (PE/CL viaja como dato del evento, no como ruta de infraestructura) — menos infraestructura que mantener, misma cobertura de servicios
...
| Alarma | DLQ | Indica |
|-------|-----|-----------|
| `clinic-scheduling-platform-country-dlq-depth` | `appointments-country-dlq` | El country worker no pudo agendar la cita en RDS |
...
Este repo forma pareja con **azure-python** y **gcp-go**: los tres implementan el mismo dominio de agendamiento de citas y Clean Architecture, los mismos 4 endpoints, distinto cloud/lenguaje — mantenidos en paridad funcional a propósito. Los cuatro proyectos fullstack de Kubernetes forman un segundo grupo así, compartiendo un dominio de Gestión de Productos en su lugar.
